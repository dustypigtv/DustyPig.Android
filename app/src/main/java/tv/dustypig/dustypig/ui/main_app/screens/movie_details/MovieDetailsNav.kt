...
object MovieDetailsNav : NavRoute<MovieDetailsViewModel> {
    ...
    const val KEY_DETAILED_PLAYLIST_ID = "KEY_DETAILED_PLAYLIST_ID"
    const val KEY_FROM_PLAYLIST_ID = "KEY_FROM_PLAYLIST_ID"
    const val KEY_PLAYLIST_UPNEXT_INDEX_ID = "KEY_PLAYLIST_UPNEXT_INDEX_ID"

    override val route =
        "movieDetails/{$KEY_MEDIA_ID}/{$KEY_DETAILED_PLAYLIST_ID}/{$KEY_FROM_PLAYLIST_ID}/{$KEY_PLAYLIST_UPNEXT_INDEX_ID}"

    fun getRoute(
        mediaId: Int,
        detailedPlaylistId: Int,
        fromPlaylist: Boolean,
        playlistUpNextIndex: Int
    ): String = route
        .replace("{$KEY_MEDIA_ID}", "$mediaId")
        .replace("{$KEY_DETAILED_PLAYLIST_ID}", "$detailedPlaylistId")
    ...
    override fun getArguments(): List<NamedNavArgument> = listOf(
        ...
        navArgument(KEY_DETAILED_PLAYLIST_ID) { type = NavType.IntType },
